...
class PlayState extends MusicBeatState
{
	...
	//new code made by: ElGatoSushi3D
	public var triggeredMode(default, set):Bool = false;
	public var shakeCam:Bool = false;
	public var bgAlt:Array<BGSprite> = [];
	...
	override public function create()
	{
		...
		if(PlayState.SONG.stage == null || PlayState.SONG.stage.length < 1) {
			switch (songName)
			{
				case 'focus' | 'soul' | 'skull' | 'vibes':
					curStage = 'graveyard';
				case 'enough':
					curStage = 'graveyardFinal';
				default:
					curStage = 'stage';
			}
		}
		...
		switch (curStage)//change this to a new function (calculate Bgs)
		{
			case 'stage': //Week 1
			...
			case 'graveyard':
					defaultCamZoom = 0.7;
					curStage = 'graveyard';
					var stageFront:BGSprite = new BGSprite("graveyardStage", -600, -200, 0.9, 0.9);
					stageFront.setGraphicSize(Std.int(stageFront.width + 50), Std.int(stageFront.height + 50));
					stageFront.updateHitbox();
					stageFront.centerOrigin();
					stageFront.antialiasing = true;
					add(stageFront);
					bgAlt.push(stageFront);

					var stageAlt:BGSprite = new BGSprite("BladeMoving-Stage", -600, -200, 0.9, 0.9, ["Backround Blade Moving0"], true);
					stageFront.setGraphicSize(Std.int(stageFront.width), Std.int(stageFront.height));
					stageFront.updateHitbox();
					stageFront.centerOrigin();
					stageAlt.antialiasing = true;
					stageAlt.visible = false;
					add(stageAlt);
					bgAlt.push(stageAlt);

					var effect:BGSprite = new BGSprite("thefunnyeffect", -600, -200, 0.9, 0.9);
					effect.setGraphicSize(Std.int(stageFront.width + 50), Std.int(stageFront.height + 50));
					effect.updateHitbox();
					effect.centerOrigin();
					effect.screenCenter();
					effect.antialiasing = true;
					effect.visible = false;
					effect.color = FlxColor.PURPLE;
					effect.scrollFactor.set(0,0);
					add(effect);
					bgAlt.push(effect);
		...
		if(curStage == 'graveyard' || curStage == 'graveyardFinal')
		{
			gf.visible = false;
		}
	...
	function set_triggeredMode(value:Bool):Bool
	{

		bgAlt[0].visible = !value;
		bgAlt[1].visible = value;
		bgAlt[2].visible = value;
		shakeCam = value;
  ...
  public function clearShaderFromCamera(cam:String){
	...
	function doSimpleSAD() // FUNNY IAMGOD
		{
			trace('SIMPLE SAD');
	
			var simpleSAD:FlxSprite = new FlxSprite().loadGraphic(Paths.image('SAD'));
	
			simpleSAD.setGraphicSize(FlxG.width, FlxG.height);
	
			simpleSAD.screenCenter();
	
			simpleSAD.cameras = [camHUD2];
	
			add(simpleSAD);
	
			new FlxTimer().start(0.2, function(tmr:FlxTimer)
			{
				trace('ended simple SAD');
				remove(simpleSAD);
			});
		}
	...
	override function stepHit()
	{
		...
		if (curSong == 'skull' && curStep != lastStepHit)
			{
				switch (curStep)
				{
					case 387:
			...
			if (curSong == 'enough' )
			{
				...
				if(curStep == 2014)
				{
					FlxTween.tween(bgAlt[2], {alpha: 0}, 0.5, {onComplete: function(twn:FlxTween)
					{
						bgAlt[2].visible = false;
						bgAlt[2].alpha = 1;//just let this here
					}});
